...
add_filter( 'wp_setup_nav_menu_item', 'vslmd_add_custom_nav_fields' );

// save menu custom fields
add_action( 'wp_update_nav_menu_item', 'vslmd_update_custom_nav_fields', 10, 3 );

// edit menu walker
add_filter( 'wp_edit_nav_menu_walker', 'vslmd_edit_walker', 10, 2 );


function vslmd_add_custom_nav_fields( $menu_item ) {
	...
	$menu_item->position = get_post_meta( $menu_item->ID, '_menu_item_position', true );
    $menu_item->show_widget_area_in_popup = get_post_meta( $menu_item->ID, '_menu_item_show_widget_area_in_popup', true );
	
	
	$menu_item->dropdown_type = get_post_meta( $menu_item->ID, '_menu_item_dropdown_type', true );
	$menu_item->col_width = get_post_meta( $menu_item->ID, '_menu_item_col_width', true );
	...
	$menu_item->bg_attachment = get_post_meta( $menu_item->ID, '_menu_item_bg_attachment', true );
 ...
 * @return      void
*/
function vslmd_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		
    $check = array('col_width', 'bg_attachment', 'bg_position', 'bg_image', 'bg_repeat', 'bg_size', 'dropdown_type', 'visibility', 'anchor', 'nolink', 'hide', 'type_menu', 'icon', 'icon_pack', 'sidebar', 'position', 'show_widget_area_in_popup');
    if(isset($_POST['vslmd_menu_options'])) {
        parse_str(urldecode($_POST['vslmd_menu_options']), $parse_array);
 ...
 * @return      void
*/
function vslmd_edit_walker($walker,$menu_id) {
...
if (!class_exists('vslmd_type1_walker_nav_menu')) {
	class vslmd_type1_walker_nav_menu extends Walker_Nav_Menu {
	...
	// add classes to ul sub-menus
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$wide_background = '';
			if(isset($vslmd_options_proya['enable_wide_manu_background']) && $vslmd_options_proya['enable_wide_manu_background'] == "yes" ){
			...
			if($item->hide == ""){
                ...
                if($depth==0 && isset($vslmd_options_proya['menu_underline_dash']) && $vslmd_options_proya['menu_underline_dash'] == "yes"){
...
if (!class_exists('vslmd_type2_walker_nav_menu')) {
	class vslmd_type2_walker_nav_menu extends Walker_Nav_Menu {
...
if (!class_exists('vslmd_type3_walker_nav_menu')) {
    class vslmd_type3_walker_nav_menu extends Walker_Nav_Menu {
...
if (!class_exists('vslmd_type4_walker_nav_menu')) {
    class vslmd_type4_walker_nav_menu extends Walker_Nav_Menu {
