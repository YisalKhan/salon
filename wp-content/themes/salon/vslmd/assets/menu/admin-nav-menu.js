(function($) {
	$(document).ready(function() {
        vslmdUpdateIconOptions();
		vslmdInitAdditionalItemOptions();
	});

	/**
	 * Function that serializes additional menu item options in a single field.
	 */
	function vslmdInitAdditionalItemOptions() {
		var navForm = $('#update-nav-menu');

		navForm.on('change', '[data-item-option]', function() {
			vslmdGenerateSerializedString();
		});
	}

	function vslmdGenerateSerializedString() {
		var dataArrayString = '';
		var navForm = $('#update-nav-menu');
		...
		if($('input[name="vslmd_menu_options"]').length) {
			$('input[name="vslmd_menu_options"]').val(encodeURIComponent(dataArrayString));
		} else {
			//generate hidden input field html with serialized string value
			var hiddenMenuItem = '<input type="hidden" name="vslmd_menu_options" value="'+encodeURIComponent(dataArrayString)+'">';
     ...
     */
    function vslmdUpdateIconOptions() {
        var navForm = $('#update-nav-menu');

        navForm.on('change', '[data-icon-pack]', function() {
            var chosenIconPack = $(this).find('option:selected').val();
            var iconDropdown   = $(this).parents('p').first().next('.vslmd-icon-select-holder').find('select');
